...
Note writing owns vault folder creation, duplicate checks by generated path or optional frontmatter URL index, default YAML/Markdown note formatting, optional template rendering, and imported filename rules.

The writer is [[src/note-writer.ts#NoteWriter]]. Frontmatter URL duplicate indexing is handled by [[src/duplicate-url-index.ts#DuplicateUrlIndex]]. Template context and fallback note formatting are handled by [[src/template-renderer.ts#buildNoteTemplateContext]], [[src/template-renderer.ts#renderNoteTemplate]], and [[src/template-renderer.ts#formatDefaultNote]]. HTML returned by readers or [[architecture#Article source fetching]] is converted by [[src/utils/html-to-markdown.ts#htmlToMarkdown]], while note filenames use an Obsidian-safe article title followed by ` - RSS ` and a short hash.
